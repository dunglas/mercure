...
(#publication)). This mirrors the reserved wildcard characters of other publish-subscribe
systems.
...
*   `type` (optional): the SSE `event` property (a specific event type). The value **MUST NOT**
    ...
    itself (see (#subscription-events)); hubs **MUST** reject a publish request whose `type` is
...
**MUST** be validated as described in (#token-validation). Every token, including a
self-issued one, **MUST** carry the claims [@!RFC9068] requires issuers to populate (`iss`,
`exp`, `aud`, `sub`, `client_id`, `iat`, and `jti`), so that any [@!RFC9068] validator can
...
token issuer: it **MUST** use a stable issuer identifier (for example, its own URL) as the
`iss` value, bound out of band to its signing key(s) in the hub's configuration (see
(#token-validation)). When an authorization server is used, the hub
...
For example, a self-issued subscriber token is a JWS with the protected header
`{"alg": "ES256", "typ": "at+jwt"}` and the following claims (see (#token-validation) for
how a hub validates them):
...
Two mechanisms are defined to present the access token to the hub, following the OAuth 2.0
...
The `access_token` URI query parameter of [@!RFC6750] section 2.3 is not part of this
specification: the OAuth 2.0 Security Best Current Practice [@!RFC9700] forbids clients from
passing access tokens in the URI query string, and hubs implementing this specification do not
accept it.

When either mechanism is used, the connection **MUST** use an encryption layer such as
HTTPS.

Per [@!RFC6750], clients **MUST NOT** use more than one mechanism to transmit the token in a
single request. [@!RFC6750] treats a request that nevertheless carries more than one mechanism as
an `invalid_request`; this specification deviates from that handling in one narrow case: when
an `Authorization` HTTP header is present, its token **MUST** be used and any cookie **MUST**
...
explicit `Authorization` header is not treated as a client protocol violation. When no
`Authorization` header is present, the cookie, if any, **MUST** be used.
...
`GET` HTTP method. However, cookies are supported and can be included even in cross-origin
...
**SHOULD** also have `SameSite=Strict` [@!I-D.ietf-httpbis-rfc6265bis]: because the publisher
and the hub share the same registrable domain (see above), requests between them are same-site
and `SameSite=Strict` does not impede them. `SameSite=Lax` **MAY** be used when the deployment
requires the cookie on qualifying top-level navigations arriving from other sites; `Lax` does
not release the cookie on cross-site subresource requests such as `EventSource` connections
[@!I-D.ietf-httpbis-rfc6265bis], so it does not enable cross-site hub access — a genuinely
cross-site deployment would require `SameSite=None; Secure` and remains subject to user-agent
third-party-cookie policies, and **SHOULD** use the `Authorization` header via `fetch()`
instead (see (#authorization-http-header)).
...
documents for hubs rather than replace them. In particular:
...
*   Hubs **MUST** select the verification key from a preconfigured or pre-trusted set and
    **MUST NOT** use key material supplied by the token itself (such as the `jwk`, `jku`, or
    ...
    Set [@!RFC7517] discovered through its metadata [@!RFC8414]. This specification defines no
    hub-specific key distribution endpoint. Each trusted key **MUST** be associated in the
    hub's configuration with the issuer identifier it belongs to — the authorization server's
    issuer identifier [@!RFC8414], or the stable identifier chosen by a self-issuing publisher.
    When the hub trusts key material from more than one issuer, it
    **MUST** verify the signature using only the key(s) associated with the token's `iss`,
    never a pooled
...
*   Hubs **MUST** be configured with their resource identifier and **MUST** verify that it
    ...
    hubs that share signing keys (see (#hub-trust)). Hubs **MUST** verify that the `iss` claim
    is present and exactly matches the issuer identifier of one of the hub's trusted issuers
    [@!RFC9068] — an authorization server (whether or not it is advertised in the hub's
    protected resource metadata, see (#discovery)) or a self-issuing publisher configured out
    of band.

[@!RFC9068] requires issuers to populate the `iss`, `exp`, `aud`, `sub`, `client_id`, `iat`,
and `jti` claims; this requirement applies to self-issuing publishers as it does to
authorization servers (see (#authorization)), so that every token is a conforming [@!RFC9068]
JWT access token. Hubs **MUST** enforce `iss`, `exp`, and `aud` as described above, per
[@!RFC9068] section 4; the remaining claims are issuer obligations that hubs use when they
rely on them.

The `sub` claim identifies the subscriber and is used to derive subscription
...
identified by the URI `https://mercure.rocks/authorization-detail`, referred to in this
...
their registration with authorization servers out of scope); as [@!RFC9396] section 2.1
...
*   `{subscriber}`: a unique identifier for the subscriber. Subscribers and publishers
    **MUST NOT** forge, supply, or override this value through query parameters, headers,
    ...
    (#token-validation)) is not sufficient on its own, because one access token can be used
    on several concurrent connections (for example, two browser tabs) whose subscription
    event topics would then collide (see (#subscriber-identifier-assignment)). A derivation
...
property (see (#publication)) — set to the reserved value `mercure`. A subscriber receiving
...
(see (#publication)), so a subscriber can trust that an event of this type was generated by the
...
of the hub URL (see (#publication) and (#subscription-events)). Examples throughout this document
...
*   `authorization_details_types_supported`: the authorization detail types the hub understands.
    ...
    `https://mercure.rocks/authorization-detail` (see (#authorization-details)).
*   `bearer_methods_supported`: the [@!RFC6750] token presentation methods the hub actually
    accepts (see (#presenting-the-access-token)): `header` [@!RFC9728]. The cookie mechanism is
    not a [@!RFC6750]
...
(#presenting-the-access-token)). Browsers, web servers, and other software may not adequately
...
generates (see (#subscription-events)), and a subscriber may route on it (for example, with
...
reject a publish request whose `type` is `mercure` (see (#publication)). Reserved-namespace
...
has cryptographically validated (see (#subscription-events)). Allowing clients to supply,
...
|--------|---------------------|--------------------|
| Authorization grant | `mercure` JWT claim with `publish` and `subscribe` arrays of topic selectors | RFC 9396 `authorization_details` entry with `type` `https://mercure.rocks/authorization-detail`, an `actions` array (`publish`, `subscribe`), and a `topics` array (see (#authorization-details)) |
...
| Reconnection query parameter | `lastEventID` | `last_event_id` (see (#reconciliation)) |
...
| Token in the URL | `authorization` query parameter | not accepted; `Authorization` header or cookie (see (#presenting-the-access-token)) |
