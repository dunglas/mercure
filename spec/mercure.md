...
*   `{subscriber}`: a unique identifier for the subscriber. Subscribers and publishers
    ...
    **MUST** incorporate the issuer: `sub` alone is unique only within
    ...
    identifier uniquely identifies the subscriber: distinct subscribers **MUST** receive distinct
    identifiers. A single subscriber holding several active subscriptions reuses the same
