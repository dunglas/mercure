...
an `invalid_request`; this specification deviates from that handling for robustness: rather than
rejecting the request, the hub **MUST** select exactly one token using the
...
*   `authorization_details_types_supported`: the authorization detail types the hub understands.
    ...
    `mercure` (see (#authorization-details)).
