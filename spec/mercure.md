...
In case of success, the HTTP response's content **MUST** be the `id` associated to this update (the one generated by the hub, if it has not been provided by the client) and a success HTTP status code **MUST** be returned.
