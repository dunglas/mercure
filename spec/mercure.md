...
* `id` (optional): the topic's revision identifier, it will be used as the SSE's `id` property, if omited the hub **MUST** generate a valid globally unique id. It `MAY` be an UUID. Even if provided, the hub **MAY** ignore the id provided by the client and generate its own id.
