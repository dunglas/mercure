...
func TestNewHubFromEnvErrorFromStream(t *testing.T) {
	os.Setenv("PUBLISHER_JWT_KEY", "foo")
	os.Setenv("JWT_KEY", "bar")
...
func createDummy() *Hub {
	return NewHub(NewLiveStream(), &Options{PublisherJWTKey: []byte("publisher"), SubscriberJWTKey: []byte("subscriber"), PublisherJWTAlgorithm: hmacSigningMethod, SubscriberJWTAlgorithm: hmacSigningMethod})
}

func createAnonymousDummy() *Hub {
	return createAnonymousDummyWithStream(NewLiveStream())
}

func createAnonymousDummyWithStream(s Stream) *Hub {
	return NewHub(s, &Options{
