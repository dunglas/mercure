...
| `public_url <url>`                     | the hub's public-facing URL — used as the base URL when the URL Pattern matcher resolves relative patterns or relative topics. Only required when subscribers use relative URL patterns against absolute topics on the hub URL (or vice-versa).     |                        |
| `subscriptions`                        | expose the subscription web API and dispatch private updates when a subscription between the Hub and a subscriber is established or closed. The topic follows the URL pattern `/.well-known/mercure/subscriptions/:matchType/:match/:subscriber`    |                        |
...
| `protocol_version_compatibility`       | version of the protocol to stay backward compatible with — enables the deprecated `topic=` query parameter, bare-string JWT matcher claims, the `/subscriptions/{topic}[/{subscriber}]` routes and the URI Template matcher by default (`7` or `8`) | disabled               |
| `matcher_types`                        | list of built-in topic matcher types to enable (`exact`, `urlpattern`, `regexp`, `uritemplate`, `cel`). `exact` is always registered implicitly.                                                                                                    | `exact, urlpattern` \* |
| `demo`                                 | enable the UI and expose demo endpoints                                                                                                                                                                                                             |                        |
| `ui`                                   | enable the UI but do not expose demo endpoints                                                                                                                                                                                                      |                        |
| `topic_selector_cache <maxSize>`       | Topic selector cache size — maximum number of cached match results. Pass `-1` to disable the cache.                                                                                                                                                 | `100000`               |
...
\* When `protocol_version_compatibility` is on, `uritemplate` is added to the
default `matcher_types` so v8 clients keep working.
