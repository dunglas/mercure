...
| `protocol_version_compatibility`       | version of the protocol to stay backward compatible with — enables the legacy `topic=` query parameter, bare-string JWT matcher claims, the `/subscriptions/{topic}[/{subscriber}]` routes and the URI Template matcher by default (`7` or `8`)  | disabled               |
