...
The [Push API](https://developer.mozilla.org/en-US/docs/Web/API/Push_API) targets *offline* devices, with notifications routed through Apple, Google, or Mozilla's push servers. The payload is small, the delivery semantics are at-most-once, and the audience is OS notification centers.

Mercure targets *connected* clients. Payloads are unbounded, deliveries are ordered with replay, and there's no third party in the path.
...
Public benchmarks: 40k concurrent on a t3.micro, well over 100k on bigger instances. Connection count is not the binding constraint for Mercure — fan-out throughput (publish × matching subscribers) usually saturates the network before connection count saturates RAM.
