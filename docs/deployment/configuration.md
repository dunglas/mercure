...
| `events_query`                             | Negotiate [HTTP Events Query](https://datatracker.ietf.org/doc/draft-gupta-httpapi-events-query/) responses (`multipart/mixed`) on `QUERY` subscribe requests, and honor the `Events` request header. Experimental. See [Subscribing](../concepts/subscribing.md#subscribing-with-http-events-query). | off                             |
