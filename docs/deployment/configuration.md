...
| `events_query`                             | Negotiate [HTTP Events Query](https://datatracker.ietf.org/doc/draft-gupta-httpapi-events-query/) responses (`multipart/mixed`) on `QUERY` subscribe requests, honor the `Events` request header, and accept [`multipart/form-data` publications](../concepts/publishing.md#publishing-binary-data). Experimental. | off                             |
