...
- **Publish rate × number of matching subscribers per topic.** A 1-publish-per-second feed to 100k subscribers is far heavier than 1k publishes per second to 100 subscribers each.
...
- Network bandwidth saturated by fan-out (publish × subscribers per topic exceeds what your NIC can deliver).
