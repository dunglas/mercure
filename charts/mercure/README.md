...
| persistence | object | `{"accessMode":"ReadWriteOnce","enabled":false,"existingClaim":"","size":"1Gi","storageClass":""}` | Switch /data from an emptyDir to a [Persistent Volume Claim](http://kubernetes.io/docs/user-guide/persistent-volumes/). The chart always mounts /data so any module that writes under `caddy.AppDataDir()` (e.g. `rate_limit`) works under `readOnlyRootFilesystem: true`. Enable persistence when you need /data to survive pod restarts, typically with the default BoltDB transport. |
...
| podSecurityContext | object | `{"seccompProfile":{"type":"RuntimeDefault"}}` | Pod [security context](https://kubernetes.io/docs/tasks/configure-pod-container/security-context/#set-the-security-context-for-a-pod). See the [API reference](https://kubernetes.io/docs/reference/kubernetes-api/workload-resources/pod-v1/#security-context) for details. `seccompProfile.type: RuntimeDefault` is set by default: it engages the container runtime's seccomp profile (blocks rare/unsafe syscalls) without affecting Mercure, and is a prerequisite for the restricted PodSecurity Standard. Override with `{}` if your cluster forbids seccompProfile on pods. |
...
| securityContext | object | `{}` | Container [security context](https://kubernetes.io/docs/tasks/configure-pod-container/security-context/#set-the-security-context-for-a-container). See the [API reference](https://kubernetes.io/docs/reference/kubernetes-api/workload-resources/pod-v1/#security-context-1) for details. For rootless setups, set `service.targetPort` to an unprivileged port (e.g. `8080`): binding 80/443 fails under `allowPrivilegeEscalation: false` because K8s does not propagate `capabilities.add` to Ambient and no_new_privs makes the binary's file capability moot on exec. The chart mounts `/config` and `/tmp` as writable volumes so `readOnlyRootFilesystem: true` works out of the box; with the default BoltDB transport, also enable persistence so `/data` is a writable PVC. |
