...
| terminationGracePeriodSeconds | int | `660` | Pod terminationGracePeriodSeconds. Must be >= the shutdown drain window so SSE subscribers drain before k8s SIGKILLs the pod. The drain window is `write_timeout` (subscribers drain at their own write deadline), or the shorter `drain_timeout` when that directive is set (via `extraDirectives`), which lets a long `write_timeout` keep a short grace period. Default = Mercure's `DefaultWriteTimeout` (600s) + 60s margin. Only applied with `RollingUpdate`; `Recreate` keeps the k8s default (30s) to minimize the gap between old pod gone and new pod ready. |
