...
// CELMatcher is the built-in CEL (Common Expression Language) matching implementation.
// CEL expressions receive a `topics` variable (list of strings) and must return a boolean.
var CELMatcher Matcher = newCELMatcherType() //nolint:gochecknoglobals
...
func newCELMatcherType() *celMatcherType {
	env, err := cel.NewEnv(
...
func (c *celMatcherType) getOrCompile(pattern string) (cel.Program, error) { //nolint:ireturn
	...
	if !loaded && err != nil {
		...
		slog.Default().Warn("CEL matcher compilation failed, pattern will be rejected on all future calls", "pattern", pattern, "error", err)
	}
