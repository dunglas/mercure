...
func TestAuthorizeMultipleAuthorizationHeaderJWKS(t *testing.T) {
	...
	claims, err := authorize(r, &jwtConfig{[]byte{}, jwt.SigningMethodRS256}, server.URL, []string{}, defaultCookieName)
	assert.EqualError(t, err, "invalid \"Authorization\" HTTP header")
	assert.Nil(t, claims)
}
...
func createJWTWithPrivateKey(t *testing.T ,priv ed25519.PrivateKey, r role, topics []string) string {
	...
	unsigned.Claims = &claims{Mercure: mercureClaim{Publish: []string{"*"}}, RegisteredClaims: jwt.RegisteredClaims{}}
